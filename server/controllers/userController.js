...
const getUserByToken = asyncHandler(async (req, res) => {
  ...
  const user = await User.findById(keyId);
  if (user) {
    res.json(user);
  } else {
    res.status(404);
...
const getDashboardInfo = asyncHandler(async (req, res) => {
  ...
  const countOpenComplaints = await Complaint.countDocuments({ action: 1 });
  const countRooms = await Room.countDocuments({});
  res.json({ countStudent, countComplaints, countRooms, countOpenComplaints });
});

export {
  authUser, deleteUser, getDashboardInfo, getUserById, getUserByToken, getUserProfile, getUsers, registerUser, updateUser, updateUserProfile
};
