...
export class BaseserviceService {
  ...
  constructor(private http: HttpClient) {
    this.cookieKey = localStorage.getItem("token")!;
  ...
  Getlist(url: string): Observable<any> {
    ...
    // if (isPlatformBrowser(this.platformId) && this.cookieKey.length > 0) {
    //   _headers = _headers.append('Authorization', `Bearer ${this.cookieKey}`);
    // }
